...
//    store phase   in the IMAG component
      ...
      CData_Dens1 = CData   + ( (Dens1Idx==-1) ? Real1Idx : Dens1Idx )*CSize3D;
      CData_Phas1 = CData_Imag1;
      CData_Real2 = CData   + Real2Idx*CSize3D;
      CData_Imag2 = CData   + Imag2Idx*CSize3D;
      CData_Dens2 = CData   + ( (Dens2Idx==-1) ? Real2Idx : Dens2Idx )*CSize3D;
      CData_Phas2 = CData_Imag2;

      FData_Real1 = IntData + Real1Idx*FSize3D;
      FData_Imag1 = IntData + Imag1Idx*FSize3D;
      FData_Dens1 = IntData + ( (Dens1Idx==-1) ? Real1Idx : Dens1Idx )*FSize3D;
      FData_Phas1 = FData_Imag1;
      FData_Real2 = IntData + Real2Idx*FSize3D;
      FData_Imag2 = IntData + Imag2Idx*FSize3D;
      FData_Dens2 = IntData + ( (Dens2Idx==-1) ? Real2Idx : Dens2Idx )*FSize3D;
      FData_Phas2 = FData_Imag2;
...
//    get the density and wrapped phase
      ...
      for (int t=0; t<CSize3D; t++)
      {
         ...
         CData_Phas1[t] = ATAN2( Im1, Re1 );
         CData_Phas2[t] = ATAN2( Im2, Re2 );
         if ( Dens1Idx == -1 ) // only need to recalculate density if it's not prepared already
         CData_Dens1[t] = Re1*Re1 + Im1*Im1;
         if ( Dens2Idx == -1 ) // only need to recalculate density if it's not prepared already
...
//    interpolate phase
      Interpolate( CData_Phas1, CSize, CStart, CRange, FData_Phas1, FSize, FStart, 1, IntScheme,
                   PhaseUnwrapping_Yes, &EnsureMonotonicity_No );
      Interpolate( CData_Phas2, CSize, CStart, CRange, FData_Phas2, FSize, FStart, 1, IntScheme,
                   PhaseUnwrapping_Yes, &EnsureMonotonicity_No );
...
//    --> apply it to density/phase instead of real/imaginary parts for better accuracy
      if ( FluIntTime )
      {
         const int TFluVarIdxList_IntTime[4] = { REAL1, IMAG1, REAL2, IMAG2 };
         const int NVar_Flu_IntTime          = 4;
               ...
               switch ( FluBC[ BC_Face[BC_Sibling] ] )
               {
                  case BC_FLU_USER:
                     Flu_BoundaryCondition_User   ( CData_Ptr,                      NVar_Flu_IntTime,
                                                    CSize[0], CSize[1], CSize[2], BC_Idx_Start, BC_Idx_End,
                                                    TFluVarIdxList_IntTime, PrepTime, dh, xyz, _REAL1|_IMAG1|_REAL2|_IMAG2, lv );
                  break;
...
#        endif
         ...
         real *CData_Real1_IntTime = CData           + 0*CSize3D;
         real *CData_Imag1_IntTime = CData           + 1*CSize3D;
         real *CData_Dens1_IntTime = CData_Real1_IntTime;
         real *CData_Phas1_IntTime = CData_Imag1_IntTime;
         real *CData_Real2_IntTime = CData           + 2*CSize3D;
         real *CData_Imag2_IntTime = CData           + 3*CSize3D;
         real *CData_Dens2_IntTime = CData_Real2_IntTime;
         real *CData_Phas2_IntTime = CData_Imag2_IntTime;

         real *FData_Dens1_IntTime = IntData_IntTime + 0*FSize3D;
         real *FData_Phas1_IntTime = IntData_IntTime + 1*FSize3D;
         real *FData_Dens2_IntTime = IntData_IntTime + 2*FSize3D;
         real *FData_Phas2_IntTime = IntData_IntTime + 3*FSize3D;

         for (int t=0; t<CSize3D; t++)
         {
            Re1 = CData_Real1_IntTime[t];
            Im1 = CData_Imag1_IntTime[t];
            Re2 = CData_Real2_IntTime[t];
            Im2 = CData_Imag2_IntTime[t];

            CData_Phas1_IntTime[t] = ATAN2( Im1, Re1 );
            CData_Dens1_IntTime[t] = Re1*Re1 + Im1*Im1;
            CData_Phas2_IntTime[t] = ATAN2( Im2, Re2 );
            CData_Dens2_IntTime[t] = Re2*Re2 + Im2*Im2;
         }

//       interpolate density
         Interpolate( CData_Dens1_IntTime, CSize, CStart, CRange, FData_Dens1_IntTime, FSize, FStart, 1, IntScheme,
                      PhaseUnwrapping_No, &EnsureMonotonicity_Yes );
         Interpolate( CData_Dens2_IntTime, CSize, CStart, CRange, FData_Dens2_IntTime, FSize, FStart, 1, IntScheme,
                      PhaseUnwrapping_No, &EnsureMonotonicity_Yes );

//       interpolate phase
         Interpolate( CData_Phas1_IntTime, CSize, CStart, CRange, FData_Phas1_IntTime, FSize, FStart, 1, IntScheme,
                      PhaseUnwrapping_Yes, &EnsureMonotonicity_No );
         Interpolate( CData_Phas2_IntTime, CSize, CStart, CRange, FData_Phas2_IntTime, FSize, FStart, 1, IntScheme,
                      PhaseUnwrapping_Yes, &EnsureMonotonicity_No );
...
//          must unwrap phase before interpolating it
            FData_Phas1_IntTime[t] = ELBDM_UnwrapPhase( FData_Phas1[t], FData_Phas1_IntTime[t] );
            FData_Phas2_IntTime[t] = ELBDM_UnwrapPhase( FData_Phas2[t], FData_Phas2_IntTime[t] );

            FData_Dens1[t] = FluWeighting     *FData_Dens1        [t]
                          + FluWeighting_IntT*FData_Dens1_IntTime[t];
            FData_Phas1[t] = FluWeighting     *FData_Phas1        [t]
                          + FluWeighting_IntT*FData_Phas1_IntTime[t];
            FData_Dens2[t] = FluWeighting     *FData_Dens2        [t]
                          + FluWeighting_IntT*FData_Dens2_IntTime[t];
            FData_Phas2[t] = FluWeighting     *FData_Phas2        [t]
                          + FluWeighting_IntT*FData_Phas2_IntTime[t];
         }
      } // FluIntTime


//    density and phase --> real and imaginary parts
      real Dens1, Dens2, Phase1, Phase2, Amp1, Amp2;

      for (int t=0; t<FSize3D; t++)
      {
         Dens1  = FData_Dens1[t];
         Phase1 = FData_Phas1[t];
         Dens2  = FData_Dens2[t];
         Phase2 = FData_Phas2[t];
