...
// remove the CM velocity
   for (int d=0; d<3; d++)    ELBDM_Vcm[d] *= ELBDM_ETA1;
...
#     pragma omp parallel for schedule( runtime )
      for (int PID=0; PID<amr->NPatchComma[lv][1]; PID++)
      {
         ...
         for (int i=0; i<PS1; i++)  {  x = x0 + i*dh;
            ...
            fluid[REAL1][k][j][i] = +R*COS(S) + I*SIN(S);
            fluid[IMAG1][k][j][i] = -R*SIN(S) + I*COS(S);
         }}}
      } // for (int PID=0; PID<amr->NPatchComma[lv][1]; PID++)

//    update the data on MPI buffer patches
      Buf_GetBufferData( lv, amr->FluSg[lv], NULL_INT, DATA_GENERAL, _REAL1|_IMAG1, Flu_ParaBuf, USELB_YES );
   } // for (int lv=0; lv<NLEVEL; lv++)
