                ...
                const int lv, double AuxArray[] )
{
   ...
   const double Gau_Const1 = 1.0 + pow(  Time / ( ELBDM_ETA1*SQR(Gau_Width) ), 2.0  );
   ...
   const double Gau_Theta2 = 0.5*pow( dr1, 2.0 )*ELBDM_ETA1*Time/(  pow( ELBDM_ETA1*SQR(Gau_Width), 2.0) + SQR(Time)  )
                             + Gau_v0*ELBDM_ETA1*dr2;

   fluid[REAL1] = Gau_Const2*cos( Gau_Theta1 + Gau_Theta2 );
   fluid[IMAG1] = Gau_Const2*sin( Gau_Theta1 + Gau_Theta2 );
