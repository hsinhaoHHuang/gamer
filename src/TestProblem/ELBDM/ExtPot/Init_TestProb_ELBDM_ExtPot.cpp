                ...
                const int lv, double AuxArray[] )
{
   ...
   const double Coeff1 = 2.0*SQR(ELBDM_ETA1)*NEWTON_G*ExtPot_M;
   const double R1     = sqrt( Coeff1*r );
   const double Coeff2 = 2.0*SQR(ELBDM_ETA2)*NEWTON_G*ExtPot_M;
   const double R2     = sqrt( Coeff2*r );

   fluid[REAL1] = ExtPot_Amp*j1( 2.0*R1 )/R1;
