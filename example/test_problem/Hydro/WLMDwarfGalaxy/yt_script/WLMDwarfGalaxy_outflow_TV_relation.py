...
def create_PhaseDiagram( ds, obj, prefix, field_type, norm, weighting_type, nbin, x_lim_min, x_lim_max, y_lim_min, y_lim_max ):
    ...
    temp_vel_profile[(field_type, weighting_type)].tofile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_z.bin'%(ds, prefix, norm, weighting_type))
    temp_vel_profile.x.tofile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_x.bin'%(ds, prefix, norm, weighting_type))
    temp_vel_profile.y.tofile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_y.bin'%(ds, prefix, norm, weighting_type))
...
def plot_PhaseDiagram( idx_start, idx_end, code, prefix, norm, weighting_type, nbin, x_lim_min, x_lim_max, y_lim_min, y_lim_max, title_time, fileout ):
...
#   plot
    ...
    for idx in range(idx_start, idx_end+1, 1)[::-1]:
        ...
        if idx == idx_end:
            xf  = np.fromfile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_x.bin'%(filename, prefix, norm, weighting_type))
            yf  = np.fromfile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_y.bin'%(filename, prefix, norm, weighting_type))
            zf  = np.fromfile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, norm, weighting_type)).reshape(len(xf),len(yf))
        else:
            zf += np.fromfile('%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram_z.bin'%(filename, prefix, norm, weighting_type)).reshape(len(xf),len(yf))
    ...
    plot_PDF( fig, 'x', idx_start, idx_end, code, prefix, v_field, weighting_type, x_lim_min, x_lim_max, nbin, units_x )
    plot_PDF( fig, 'y', idx_start, idx_end, code, prefix, 'T',     weighting_type, y_lim_min, y_lim_max, nbin, units_y )
    fig.savefig( 'fig_%s%s_Temp-Vel%s_%s_weighted_PhaseDiagram.png'%(fileout, prefix, norm, weighting_type) )
    plt.close()
